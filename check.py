...
# Seconds the untimed warm-up run may take before it is abandoned.
WARM_UP_TIMEOUT = 60
...
def warm_up(executable, infile):
    """Run the binary once on a temporary `000` input, untimed and unreported.

    The first run of a newly built executable pays a one-off cost before main()
    is even reached: the OS validates the new file and scans it, then caches
    that verdict per file — ~180 ms on macOS, ~25-45 ms for Defender on Windows.
    `compile()` produces a new binary on every run, so without this the cost
    lands on whichever test runs first and reads as solve time.

    The cache is keyed by the binary, so only a run of this same executable can
    absorb it: building a separate warm-up program leaves the real one just as
    cold. The input is a temporary copy of the first test's, since an empty
    stdin hangs a solution that reads until it has its numbers. Nothing here is
    timed, compared or counted — a failure to run is left for the timed run to
    report.
    """
    temp_dir = pathlib.Path(tempfile.mkdtemp(prefix="check_warmup_"))
    warm_input = temp_dir / f"000.{infile.suffix.lstrip('.')}"
    try:
        shutil.copyfile(infile, warm_input)
        with open(warm_input, "r") as fin:
            subprocess.run([executable], stdin=fin, capture_output=True,
                           text=True, check=False, timeout=WARM_UP_TIMEOUT)
    except Exception:
        pass
    finally:
...
def main():
    ...
    parser.add_argument("--in", dest="in_ext", type=str, default="in",
                        help="extension of the input files (e.g. in, inp)")
    parser.add_argument("--out", dest="out_ext", type=str, default="out",
                        help="extension of the expected-output files (e.g. out, md)")
    ...
    try:
        with tests_directory(folder_path) as tests_dir:
            if tests_dir is None:
                ...
                sys.exit(1)
            in_files = sorted(tests_dir.glob(f"*.{in_ext}"))
            if not in_files:
                print(f"No *.{in_ext} files found in {tests_dir}")
                sys.exit(1)
            test_count = len(in_files)
            pass_count = 0
            exe_path = str(pathlib.Path(executable).resolve())
            print("Warming up on a temporary 000 ..")
            warm_up(exe_path, in_files[0])
            print(f"{'Verdict':<8}{'Test':>8}{'Time':>12}")
            time.perf_counter()  # warm up timer
            for infile in in_files:
                outfile = infile.with_name(f"{infile.stem}.{out_ext}")
