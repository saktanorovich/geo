...
python check.py ./<problem> --str --out md   # expected outputs are .md, not .out
...
- The right `--tol` is per-problem and comes from the statement. `README.md` lists the
  exact command for each — read it there rather than guessing, so the two cannot drift
  apart. `REPORT.md` carries the verdicts and per-test timings.
- `check.py` compiles with `g++ -std=c++17 -O2` to a binary always named `program`,
  feeds each `tests/NNN.in` on stdin and diffs stdout against `tests/NNN.out`. Tests use
  a 3-digit schema. `--in` and `--out` name those two extensions when a problem departs
  from them — `quad` keeps its expected output in `tests/NNN.md` and runs as
  `--str --out md`. A leading dot is accepted either way.
...
failing, and record the red verdict in `REPORT.md` with the expected-versus-reported
figures — a red suite that is explained is better than a green one that hides a known
bug. `REPORT.md` holds verdicts, timings and those figures only; the reasoning belongs
in the change itself, not in the report.
